...
WITH seed AS (
  SELECT
    n,
    (md5('loverage-seed-woman-' || n::text))::uuid AS id,
    (ARRAY[
      'Sophia','Mariam','Layla','Nour','Sara','Yasmin','Amina','Hana','Lina','Salma',
      'Dalia','Farah','Leen','Rania','Aya','Reem','Maya','Dana','Jana','Hala'
    ])[n] AS name
...
WITH seed_ids AS (
  SELECT (md5('loverage-seed-woman-' || n::text))::uuid AS id
  FROM generate_series(1, 20) AS n
)
...
SELECT
  id, name, 'Female', 'Islam',
  'Warm, thoughtful, and looking for a genuine connection.',
  (ARRAY['Dubai','Abu Dhabi','Sharjah','Ajman'])[1 + ((n - 1) % 4)],
  'AE', 'approved', 'active', 100,
  23 + ((n - 1) % 12),
  (ARRAY['Architect','Teacher','Doctor','Designer','Entrepreneur'])[1 + ((n - 1) % 5)],
  ARRAY['Warm','Thoughtful','Family-oriented'],
  (n % 5 = 0), false,
  now() - make_interval(days => n), now(), now(), now()
FROM seed;

WITH seed AS (
  SELECT n, (md5('loverage-seed-woman-' || n::text))::uuid AS id
  FROM generate_series(1, 20) AS n
)
...
SELECT
  (md5('loverage-seed-photo-woman-' || n::text))::uuid,
  id,
  'https://randomuser.me/api/portraits/women/' || (n + 10) || '.jpg',
  true, 0, 'approved'
FROM seed;

UPDATE public.profiles p
SET main_photo_id = (md5('loverage-seed-photo-woman-' || s.n::text))::uuid
FROM generate_series(1, 20) AS s(n)
WHERE p.id = (md5('loverage-seed-woman-' || s.n::text))::uuid;
